...
def validate(repo_root: Path) -> None:
    ...
    require(ui_apply, "SAURUS_ANDROID_XML_ATTRIBUTE_PATCH_V5", "XML attribute upsert revision")
    require(ui_apply, "def upsert_xml_attribute_in_start_tag", "XML attribute upsert helper")
    require(ui_apply, "Accessibility XML duplicate-attribute regression", "duplicate XML attribute regression test")
    require(ui_apply, "SAURUS_ANDROID_ADAPTIVE_LAUNCHER_V1", "adaptive launcher patch")
    require(ui_apply, "def patch_launcher_resources", "adaptive launcher helper")
    require(ui_apply, "SAURUS_ANDROID_INCOMING_ACCEPT_V1", "incoming accept action patch")
    require(ui_apply, "def patch_incoming_accept_dialog_content", "incoming accept helper")
    require(ui_verify, "label: const Text('Aceitar')", "visible incoming accept verification")
    require(ui_verify, "ET.parse(config_path)", "parsed accessibility XML verification")
